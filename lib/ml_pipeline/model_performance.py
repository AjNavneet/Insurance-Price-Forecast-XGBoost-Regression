...
# Function to compare the performance of a base model and a new model
def compare_model_performance(base_perf, new_perf):
    ...
    for metric, base_value in base_perf.items():
        ...
        results.loc[metric] = [
            base_value, new_value, new_value - base_value, round(100 * (new_value - base_value) / base_value, 2)
        ]
    return results
