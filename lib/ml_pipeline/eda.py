...
# Function to plot histograms for each feature in the dataset
def plot_histograms(X, height=1200):
    fig = make_subplots(rows=X.shape[1], cols=1)
    for i, col in enumerate(X.columns):
        # Add a histogram trace for each feature
        fig.add_trace(
            go.Histogram(
                x=X[col],
                name=col,
...
# Function to plot univariate numeric data against the target variable
def plot_univariate_numeric(X, y):
    fig = make_subplots(rows=X.shape[1], cols=1)
    for i, col in enumerate(X.columns):
        # Add a scatter plot for each numeric feature
        fig.add_trace(
            go.Scatter(
                x=X[col],
                y=y,
...
# Function to plot univariate categorical data against the target variable
...
# Function to plot a heatmap of feature interactions
...
# Function to plot paired box plots for feature pairs
...
# Function to plot paired scatter plots for feature pairs
...
# Function to plot residuals as a histogram
def plot_residuals(y_true, y_pred):
    residuals = y_true - y_pred
    fig = px.histogram(residuals)
    fig.show()

# Function to plot Pearson correlation with respect to the target variable
